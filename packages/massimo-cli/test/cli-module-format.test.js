...
test('module format - no package.json (defaults to ESM)', async t => {
  ...
  deepEqual(files.sort(), ['movies.d.mts', 'movies.mjs', 'movies.schema.graphql', 'package.json'])

  // Check generated package.json
  const pkg = JSON.parse(await fs.readFile(join(dir, 'movies', 'package.json'), 'utf-8'))
  deepEqual(pkg, {
    name: 'movies',
    types: './movies.d.mts',
    type: 'module',
...
test('module format - package.json without type field (defaults to CommonJS)', async t => {
  ...
  await fs.writeFile('./package.json', JSON.stringify({ name: 'my-project' }))
  ...
  const files = await fs.readdir(join(dir, 'movies'))
  deepEqual(files.sort(), ['movies.cjs', 'movies.d.ts', 'movies.schema.graphql', 'package.json'])
